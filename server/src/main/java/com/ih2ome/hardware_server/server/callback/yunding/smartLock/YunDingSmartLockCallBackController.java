...
import com.ih2ome.sunflower.entity.narcissus.SmartHouseMapping;
...
//                break;
    ...
    private void deviceUninstall(CallbackRequestVo apiRequestVO) {
        ...
        if (detail.getString("type").equals("1")) {
            smartLockGatewayService.uninstallSmartLockGateway(apiRequestVO.getUuid());
        } else if (detail.getString("type").equals("4")) {
            smartLockService.uninstallSmartLock(apiRequestVO.getUuid());
        }
    }
     ...
     * @param apiRequestVO
     */
    private void deviceInstall(CallbackRequestVo apiRequestVO) {
        JSONObject detail = apiRequestVO.getDetail().getJSONObject("detail");
        String uuid = apiRequestVO.getUuid();
        String homeId=apiRequestVO.getHome_id();
        String roomId=apiRequestVO.getRoom_id();
        SmartHouseMappingVO smartHouseMappingVO=new SmartHouseMappingVO();
        smartHouseMappingVO.setThirdHomeId(homeId);
        smartHouseMappingVO.setThirdRoomId(roomId);
        //网关
        if (detail.getString("type").equals("1")) {
            smartLockGatewayService.installSmartLockGateway(homeId,uuid);
        } else if (detail.getString("type").equals("4")) {
            smartLockService.installSmartLock(homeId,roomId,uuid);
        }
    }
